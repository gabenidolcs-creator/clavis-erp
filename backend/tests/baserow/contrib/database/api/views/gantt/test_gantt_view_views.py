...
def test_list_rows(api_client, data_fixture):
    ...
    gantt_2 = data_fixture.create_gantt_view()

    model = gantt.table.get_model()
    ...
    url = reverse("api:database:views:gantt:list", kwargs={"view_id": 999})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    assert response.status_code == HTTP_404_NOT_FOUND
    assert response.json()["error"] == "ERROR_GANTT_DOES_NOT_EXIST"

    url = reverse("api:database:views:gantt:list", kwargs={"view_id": gantt_2.id})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    assert response.status_code == HTTP_400_BAD_REQUEST
    assert response.json()["error"] == "ERROR_USER_NOT_IN_GROUP"

    url = reverse("api:database:views:gantt:list", kwargs={"view_id": gantt.id})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
...
def test_list_rows_applies_sorts_and_filters(api_client, data_fixture):
    """AC #1 — existing view sorts and filters apply to the rows on the gantt."""
    ...
    # Sort applies.
    sort = data_fixture.create_view_sort(view=gantt, field=text_field, order="ASC")
    url = reverse("api:database:views:gantt:list", kwargs={"view_id": gantt.id})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
    ...
    view_filter = data_fixture.create_view_filter(
        view=gantt, field=text_field, value="Green"
    )
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
...
@pytest.mark.django_db
def test_patch_gantt_view_date_fields(api_client, data_fixture):
    ...
    url = reverse("api:database:views:item", kwargs={"view_id": gantt.id})
...
@pytest.mark.django_db
def test_patch_gantt_view_timescale_persists(api_client, data_fixture):
    """AC #4 — the zoom level persists through the generic view PATCH."""
    ...
    # Re-fetch the view to confirm per-view persistence (AC #4).
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    assert response.status_code == HTTP_200_OK
...
@pytest.mark.django_db
def test_patch_gantt_view_rejects_invalid_timescale(api_client, data_fixture):
    """AC #4 — an invalid zoom level is rejected by the serializer ChoiceField."""
...
@pytest.mark.django_db
def test_patch_gantt_view_rejects_non_date_field(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_patch_gantt_view_field_options_hidden_persists(api_client, data_fixture):
    """
    ...
    There is no Gantt-specific field-options endpoint; the generic one is used.
    """
    ...
    field_options_url = reverse(
        "api:database:views:field_options", kwargs={"view_id": gantt.id}
    )
    response = api_client.patch(
        field_options_url,
    ...
    list_url = reverse("api:database:views:gantt:list", kwargs={"view_id": gantt.id})
    response = api_client.get(
        list_url,
...
@pytest.mark.django_db
def test_patch_gantt_view_field_options_order_persists(api_client, data_fixture):
    """
