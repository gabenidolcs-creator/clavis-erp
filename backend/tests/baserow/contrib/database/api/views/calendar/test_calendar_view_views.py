...
def test_list_rows(api_client, data_fixture):
    ...
    calendar = data_fixture.create_calendar_view(table=table)
    calendar_2 = data_fixture.create_calendar_view()

    model = calendar.table.get_model()
    ...
    url = reverse("api:database:views:calendar:list", kwargs={"view_id": 999})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    assert response.status_code == HTTP_404_NOT_FOUND
    assert response.json()["error"] == "ERROR_CALENDAR_DOES_NOT_EXIST"

    url = reverse("api:database:views:calendar:list", kwargs={"view_id": calendar_2.id})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    assert response.status_code == HTTP_400_BAD_REQUEST
    assert response.json()["error"] == "ERROR_USER_NOT_IN_GROUP"

    url = reverse("api:database:views:calendar:list", kwargs={"view_id": calendar.id})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
...
def test_list_rows_applies_sorts_and_filters(api_client, data_fixture):
    """AC #4 — existing view sorts and filters apply to the rows on the calendar."""
    ...
    # Sort applies.
    sort = data_fixture.create_view_sort(view=calendar, field=text_field, order="ASC")
    url = reverse("api:database:views:calendar:list", kwargs={"view_id": calendar.id})
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
    ...
    view_filter = data_fixture.create_view_filter(
        view=calendar, field=text_field, value="Green"
    )
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
...
@pytest.mark.django_db
def test_patch_calendar_view_date_field(api_client, data_fixture):
    """AC #3 — the positioning date field can be set and is returned on the view."""
    ...
    url = reverse("api:database:views:item", kwargs={"view_id": calendar.id})
    ...
    response = api_client.get(url, **{"HTTP_AUTHORIZATION": f"JWT {token}"})
    assert response.status_code == HTTP_200_OK
    assert response.json()["date_field"] == date_field.id


@pytest.mark.django_db
def test_patch_calendar_view_end_date_field(api_client, data_fixture):
    """AC #1 — the optional end date field is set via the generic view PATCH."""
    ...
    response = api_client.patch(
        url,
        {"date_field": date_field.id, "end_date_field": end_date_field.id},
        format="json",
        **{"HTTP_AUTHORIZATION": f"JWT {token}"},
    )
    response_json = response.json()
...
@pytest.mark.django_db
def test_patch_calendar_view_rejects_non_date_field(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_patch_calendar_view_field_options_hidden_persists(api_client, data_fixture):
    """
    ...
    There is no Calendar-specific field-options endpoint; the generic one is used.
    """
    ...
    field_options_url = reverse(
        "api:database:views:field_options", kwargs={"view_id": calendar.id}
    )
    response = api_client.patch(
        field_options_url,
    ...
    list_url = reverse(
        "api:database:views:calendar:list", kwargs={"view_id": calendar.id}
    )
    response = api_client.get(
        list_url,
...
@pytest.mark.django_db
def test_patch_calendar_view_field_options_order_persists(api_client, data_fixture):
    """
