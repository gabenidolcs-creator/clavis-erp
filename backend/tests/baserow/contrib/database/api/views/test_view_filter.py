...
@pytest.mark.django_db
def test_create_filter_personal_view_non_owner_returns_401(api_client, data_fixture):
    """Non-owner POST of a filter on a personal view returns 401; owner POST succeeds."""
    user, token = data_fixture.create_user_and_token()
    ...
    payload = {"field": field.id, "type": "equal", "value": "test"}

    response = api_client.post(
        reverse("api:database:views:list_filters", kwargs={"view_id": personal_view.id}),
        payload,
        format="json",
