...
class CalendarViewType(ViewType):
    ...
    allowed_fields = ["date_field", "end_date_field"]
    field_options_allowed_fields = ["hidden", "order"]
    serializer_field_names = ["date_field", "end_date_field"]
    serializer_field_overrides = {
    ...
    def prepare_values(self, values, table, user):
        """
        Check that the provided date field and the optional end date field are
        date-representable fields that belong to the same table.
        """

        for name in ("date_field", "end_date_field"):
            if values.get(name, None) is not None:
                if isinstance(values[name], int):
                    values[name] = Field.objects.get(pk=values[name])

                field_type = field_type_registry.get_by_model(values[name].specific)
                if not field_type.can_represent_date(values[name]):
                    raise IncompatibleField(
                        "The provided field cannot be used to position rows on the "
                        "calendar, because it cannot represent a date."
                    )
                elif values[name].table_id != table.id:
                    raise FieldNotInTable(
    ...
    def after_fields_type_change(self, fields):
        # If a positioning field is changed to a type that can no longer
        # represent a date, the reference must be nulled out so the calendar does
        # not point at a field that has no meaningful date value.
        fields_cannot_represent_date = [
            field
            for field in fields
            if not field_type_registry.get_by_model(
                field.specific_class
            ).can_represent_date(field)
        ]
        if len(fields_cannot_represent_date) > 0:
            ids = [f.id for f in fields_cannot_represent_date]
            CalendarView.objects.filter(date_field_id__in=ids).update(
                date_field_id=None
            )
            CalendarView.objects.filter(end_date_field_id__in=ids).update(
    ...
    def export_serialized(
        ...
        if calendar.end_date_field_id:
            serialized["end_date_field_id"] = calendar.end_date_field_id
    ...
    def import_serialized(
        ...
        if serialized_copy.get("end_date_field_id", None):
            serialized_copy["end_date_field_id"] = id_mapping["database_fields"][
                serialized_copy["end_date_field_id"]
            ]
    ...
    def view_created(self, view):
        """
        When a calendar view is created, we want to set the first three fields as
        visible.
        """
    ...
    def export_prepared_values(self, view: CalendarView) -> Dict[str, Any]:
        """
        Add `date_field` and `end_date_field` to the exportable fields.

        :param view: The calendar view to export.
        :return: The prepared values.
        """
    ...
    def get_visible_field_options_in_order(self, calendar_view: CalendarView):
        return (
            ...
            .filter(
                ...
                | Q(field__id=calendar_view.end_date_field_id)
            )
            .order_by("order", "field__id")
        )
    ...
    def get_hidden_fields(
        ...
        for field in fields:
            # The date field and the end date field are always visible because the
            # calendar depends on their values to position the rows.
            if field.id == view.date_field_id:
                continue
            if field.id == view.end_date_field_id:
                continue
    ...
    def after_field_delete(self, field: Field) -> None:
        # Null the date field references so the calendar does not point at a
        # trashed field. SET_NULL on the FK only fires on a hard delete, but
        # Baserow trashes (soft-deletes) fields, so this must be done explicitly.
        CalendarView.objects.filter(date_field_id=field.id).update(date_field_id=None)
        CalendarView.objects.filter(end_date_field_id=field.id).update(
            end_date_field_id=None
        )
...
class TimelineViewType(ViewType):
    ...
    serializer_field_overrides = {
        ...
        "timescale": serializers.ChoiceField(
    ...
    def prepare_values(self, values, table, user):
        """
        Check that the provided start and end date fields are date-representable
        fields that belong to the same table.
        """

        for name in ("start_date_field", "end_date_field"):
            if values.get(name, None) is not None:
                if isinstance(values[name], int):
                    values[name] = Field.objects.get(pk=values[name])
                ...
                if not field_type.can_represent_date(values[name]):
                    raise IncompatibleField(
                        "The provided field cannot be used to position rows on the "
                        "timeline, because it cannot represent a date."
                    )
                elif values[name].table_id != table.id:
                    raise FieldNotInTable(
    ...
    def after_fields_type_change(self, fields):
        # If a positioning field is changed to a type that can no longer
        # represent a date, the reference must be nulled out so the timeline does
        # not point at a field that has no meaningful date value.
        fields_cannot_represent_date = [
            field
            for field in fields
        ...
        if len(fields_cannot_represent_date) > 0:
            ids = [f.id for f in fields_cannot_represent_date]
            TimelineView.objects.filter(start_date_field_id__in=ids).update(
                start_date_field_id=None
            )
            TimelineView.objects.filter(end_date_field_id__in=ids).update(
                end_date_field_id=None
            )
    ...
    def import_serialized(
        ...
        if serialized_copy.get("start_date_field_id", None):
            serialized_copy["start_date_field_id"] = id_mapping["database_fields"][
                serialized_copy["start_date_field_id"]
            ]
        if serialized_copy.get("end_date_field_id", None):
            serialized_copy["end_date_field_id"] = id_mapping["database_fields"][
                serialized_copy["end_date_field_id"]
    ...
    def view_created(self, view):
        """
        When a timeline view is created, we want to set the first three fields as
        visible.
        """
    ...
    def export_prepared_values(self, view: TimelineView) -> Dict[str, Any]:
        """
        Add `start_date_field`, `end_date_field` and `timescale` to the
        exportable fields.

        :param view: The timeline view to export.
        :return: The prepared values.
        """
    ...
    def get_visible_field_options_in_order(self, timeline_view: TimelineView):
        return (
            ...
            .filter(
                Q(hidden=False)
                | Q(field__id=timeline_view.start_date_field_id)
                | Q(field__id=timeline_view.end_date_field_id)
            )
            .order_by("order", "field__id")
        )
    ...
    def get_hidden_fields(
        ...
        for field in fields:
            # The start and end date fields are always visible because the
            # timeline depends on their values to position the rows.
            if field.id == view.start_date_field_id:
                continue
            if field.id == view.end_date_field_id:
                continue
    ...
    def after_field_delete(self, field: Field) -> None:
        # Null the date field references so the timeline does not point at a
        # trashed field. SET_NULL on the FK only fires on a hard delete, but
        # Baserow trashes (soft-deletes) fields, so this must be done explicitly.
...
class GanttViewType(ViewType):
    type = "gantt"
    model_class = GanttView
    field_options_model_class = GanttViewFieldOptions
    field_options_serializer_class = GanttViewFieldOptionsSerializer
    allowed_fields = ["start_date_field", "end_date_field", "timescale"]
    field_options_allowed_fields = ["hidden", "order"]
    serializer_field_names = ["start_date_field", "end_date_field", "timescale"]
    serializer_field_overrides = {
    ...
    def get_api_urls(self):
        from baserow.contrib.database.api.views.gantt import urls as api_urls

        return [
            path("gantt/", include(api_urls, namespace=self.type)),
        ]
        ...
        for name in ("start_date_field", "end_date_field"):
            if values.get(name, None) is not None:
                ...
                if not field_type.can_represent_date(values[name]):
                    raise IncompatibleField(
                        "The provided field cannot be used to position rows on the "
                        "gantt, because it cannot represent a date."
                    )
                elif values[name].table_id != table.id:
                    raise FieldNotInTable(
                        "The provided date field id does not belong to the gantt "
                        "view's table."
                    )
    ...
    def after_fields_type_change(self, fields):
        # If a positioning field is changed to a type that can no longer
        # represent a date, the reference must be nulled out so the gantt does
        # not point at a field that has no meaningful date value.
        fields_cannot_represent_date = [
            field
            for field in fields
        ...
        if len(fields_cannot_represent_date) > 0:
            ids = [f.id for f in fields_cannot_represent_date]
            GanttView.objects.filter(start_date_field_id__in=ids).update(
                start_date_field_id=None
            )
            GanttView.objects.filter(end_date_field_id__in=ids).update(
                end_date_field_id=None
            )
    ...
    def export_serialized(
        ...
        """
        Adds the serialized gantt view options to the exported dict.
        """

        serialized = super().export_serialized(
            gantt, import_export_config, cache, files_zip, storage
        )

        if gantt.start_date_field_id:
            serialized["start_date_field_id"] = gantt.start_date_field_id
        if gantt.end_date_field_id:
            serialized["end_date_field_id"] = gantt.end_date_field_id
        serialized["timescale"] = gantt.timescale

        serialized_field_options = []
        for field_option in gantt.get_field_options():
            serialized_field_options.append(
                {
                    "id": field_option.id,
                    "field_id": field_option.field_id,
    ...
    def import_serialized(
        ...
        """
        Imports the serialized gantt view field options.
        """
        ...
        gantt_view = super().import_serialized(
            table,
            serialized_copy,
            import_export_config,
            id_mapping,
        ...
        if gantt_view is not None:
            if "database_gantt_view_field_options" not in id_mapping:
                id_mapping["database_gantt_view_field_options"] = {}
            ...
            for field_option in field_options:
                ...
                field_option_object = GanttViewFieldOptions.objects.create(
                    gantt_view=gantt_view, **field_option_copy
                )
                id_mapping["database_gantt_view_field_options"][field_option_id] = (
                    field_option_object.id
                )
    ...
    def view_created(self, view):
        """
        When a gantt view is created, we want to set the first three fields as
        visible.
        """
        ...
        if ids_to_update:
            GanttViewFieldOptions.objects.filter(id__in=ids_to_update).update(
                hidden=False
            )

    def export_prepared_values(self, view: GanttView) -> Dict[str, Any]:
        """
        Add `start_date_field`, `end_date_field` and `timescale` to the
        exportable fields.

        :param view: The gantt view to export.
        :return: The prepared values.
        """
    ...
    def get_visible_field_options_in_order(self, gantt_view: GanttView):
        return (
            gantt_view.get_field_options(create_if_missing=True)
            .filter(
                Q(hidden=False)
                | Q(field__id=gantt_view.start_date_field_id)
                | Q(field__id=gantt_view.end_date_field_id)
            )
            .order_by("order", "field__id")
        )
    ...
    def get_hidden_fields(
        ...
        hidden_field_ids = set()
        fields = view.table.field_set.all()
        field_options = view.ganttviewfieldoptions_set.all()
        ...
        for field in fields:
            # The start and end date fields are always visible because the
            # gantt depends on their values to position the rows.
            if field.id == view.start_date_field_id:
                continue
            if field.id == view.end_date_field_id:
                continue
    ...
    def enhance_queryset(self, queryset):
        return queryset.prefetch_related("ganttviewfieldoptions_set")

    def after_field_delete(self, field: Field) -> None:
        # Null the date field references so the gantt does not point at a
        # trashed field. SET_NULL on the FK only fires on a hard delete, but
        # Baserow trashes (soft-deletes) fields, so this must be done explicitly.
        GanttView.objects.filter(start_date_field_id=field.id).update(
            start_date_field_id=None
        )
