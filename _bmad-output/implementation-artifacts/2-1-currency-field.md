...
- All 14 backend tests pass; all 8 frontend tests pass (expanded during QA review).
...
- `backend/src/baserow/contrib/database/migrations/0216_currencyfield_alter_formview_mode.py`
- `backend/tests/baserow/contrib/database/field/test_currency_field_type.py`
- `e2e-tests/tests/database/currency_field.spec.ts`
- `web-frontend/locales/en.json`
- `web-frontend/modules/database/components/field/FieldCurrencySubForm.vue`
...
- `web-frontend/test/unit/database/currencyFieldType.spec.js`
...
**HIGH (resolved):**
- H1: `web-frontend/modules/database/locales/en.json` had duplicate `fieldCurrencySubForm` key appended at end of file (line 1215 of committed version). QA session already removed the duplicate in the working copy. Fixed by this commit.
- H2: `CurrencyFieldType.get_export_value` used thread-unsafe try/finally mutation of `field.number_prefix` to inject the symbol before calling `super()`. Since `prepare_values` already maps `currency_symbol → number_prefix` on save, the override was redundant and racy under concurrent requests. Removed entire override; parent `NumberFieldType.get_export_value` now handles formatting correctly via the persisted `number_prefix`. Fixed by this commit.

**MEDIUM:**
- M1: Migration `0216_currencyfield_alter_formview_mode` bundles an unrelated `AlterField` on `formview.mode` (auto-generated by Django's lazy-choices serialization). The choices are serialized as a string `"[('form', 'form')]"` — no DB schema change, no behavioral regression. Accepted as-is; removing would cause Django to regenerate on next `makemigrations`.

**LOW:**
- L1: `getIconClass` returns `'baserow-icon-hashtag'` — story spec noted `'baserow-icon-currency' // or reuse a suitable existing icon`. Hashtag icon is an acceptable reuse.
- L2: Completion notes cited 6 backend / 4 frontend tests; QA session expanded to 14 / 8. Updated in this review commit.

### AC Verification

- AC1 (create, configure, store): ✅ `CurrencyField` model, `CurrencyFieldType`, `FieldCurrencySubForm.vue`, API tests confirm.
- AC2 (numeric sort): ✅ Inherited from `NumberFieldType`; `DecimalField` in PostgreSQL sorts numerically. `test_currency_field_sorts_numeric` verifies.
- AC3 (API round-trip): ✅ `test_currency_field_api_round_trip` confirms GET/PATCH/POST with `currency_symbol` and `number_decimal_places`.

### Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2026-06-07 | 2.1.1 | Review pass: populate file list, fix i18n duplicate, remove thread-unsafe get_export_value override, update test counts, approve | gabenidolcs |
