#!/usr/bin/env bash
# Batch: resume failed stories from batch-3.14-6.4
# - 4.4: review-only (create/dev/auto already done; review failed max cycles)
# - 4.5-6.3: full pipeline (create failed all 5 attempts)
...
EPIC="1"
LOG_FILE="_bmad-output/story-automator/batch-4.4-review-6.3-$(date +%Y%m%d-%H%M%S).log"

# Stories needing full pipeline (create failed)
FULL_STORIES=(4.5 4.6 5.1 5.2 5.3 5.4 6.1 6.2 6.3)
...
# ================================================================
log "=== BATCH RUN STARTED: 4.4 review-only + 4.5-6.3 full pipeline ==="
log "=== Models: Sonnet create/dev/auto | Haiku review ==="

# ================================================================
# PART 1: Story 4.4 — review-only (skip create/dev/auto)
# ================================================================
log "--- Story 4.4 (review-only) ---"

resolve_agent "review" "4.4"
log "[4.4] review: START (model=${primary_model:-default})"
review_ok=false
for cycle in 1 2 3 4 5; do
    result=$(spawn_review "4.4" "$cycle" "$primary_agent" "$primary_model")
    final=$(echo "$result" | jq -r '.final_state // "unknown"')
    if [ "$final" = "completed" ]; then
        review_ok=true; break
    fi
    log "[4.4] review cycle $cycle: $final"
    [ $cycle -lt 5 ] && sleep 15
done
if $review_ok; then
    update_progress "4.4" "done" "done" "done" "done" "⏳" "in-progress"
    log "[4.4] review: DONE"

    title=$(get_title "4.4")
    log "[4.4] commit: '$title'"
    commit=$("$SCRIPTS" commit-story --repo "." --story "4.4" --title "$title" 2>&1 || echo '{"ok":false,"error":"commit_failed"}')
    if echo "$commit" | grep -q '"ok":true'; then
        update_progress "4.4" "done" "done" "done" "done" "done" "done"
        echo "- **[$(date -u +%Y-%m-%dT%H:%M:%SZ)]** Story 4.4: ✅ complete" >> "$STATE_FILE"
        log "[4.4] ✅ COMPLETE"
    else
        err=$(echo "$commit" | jq -r '.error // "unknown"' 2>/dev/null || echo "unknown")
        update_progress "4.4" "done" "done" "done" "done" "fail" "error"
        log "[4.4] commit FAILED: $err"
    fi
else
    update_progress "4.4" "done" "done" "done" "fail" "⏳" "error"
    log "[4.4] review: FAILED (max cycles)"
fi

# ================================================================
# PART 2: Stories 4.5-6.3 — full pipeline
# ================================================================
TOTAL=${#FULL_STORIES[@]}
for idx in "${!FULL_STORIES[@]}"; do
    story_id="${FULL_STORIES[$idx]}"
    n=$((idx + 1))
    log "--- Story $n/$TOTAL (full): $story_id ---"
